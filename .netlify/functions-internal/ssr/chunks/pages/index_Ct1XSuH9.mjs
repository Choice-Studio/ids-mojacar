...
const _imports_0$1 = "/ids-logo--blue.svg";
...
function _sfc_ssrRender$4(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  _push(`<header${
    ssrRenderAttrs(mergeProps({ class: "absolute top-0 left-0 flex lg:hidden justify-between items-center w-full mx-auto z-10 wrapper py-2" }, _attrs))
  }><div class="w-12 h-12 grid place-items-center"><a href="/"><img${
    ssrRenderAttr("src", _imports_0$1)
  } alt="IDS logo" class="header__logo"></a></div><img${
    ssrRenderAttr("src", _imports_1)
  } alt="menu icon" class="w-8 header__burger-menu"><div class="${
    ssrRenderClass([[$data.isNavHidden ? 'nav--hidden' : 'nav-visible'], "origin-top w-full bg-primary top-0 left-0 h-screen px-4 py-12 text-center flex flex-col justify-center items-center gap-16 fixed"])
  }"><img${
    ssrRenderAttr("src", _imports_2)
  } alt="close menu icon" class="w-6 header__burger-menu absolute top-4 right-4"><!--[-->`);
  ssrRenderList($data.links, (link) => {
    _push(`<div class="border-t-white border-solid border-t-1"><a${
      ssrRenderAttr("href", link.pageHREF)
    } class="${
      ssrRenderClass([[link.pageName == $props.pageName ? 'highlight--dot' : ''], "header-text text-white text-2xl"])
    }">${
      ssrInterpolate(link.pageName)
    }</a></div>`);
  });
  _push(`<!--]--></div></header>`);
}
...
_sfc_main$4.setup = (props, ctx) => {
  ...
  ;(ssrContext.modules || (ssrContext.modules = new Set())).add("src/layouts/Header.vue");
